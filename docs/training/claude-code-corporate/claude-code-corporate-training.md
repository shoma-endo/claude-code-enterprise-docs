...
> [!NOTE]
> 2025年の調査では、日本企業の約56%（約6割）が生成AIを業務に活用しています。一方、期待を大きく上回る効果を実感している企業は約10%にとどまり（米国は45%）、効果創出が課題となっています（[PwC Japan 生成AI実態調査 2025](https://www.pwc.com/jp/ja/knowledge/thoughtleadership/generative-ai-survey2025.html)）。

### 3. 法人利用の安全ルール

Claude Code はローカル PC 上で動作しますが、プロンプトとレスポンスは Anthropic の API を経由します（[セキュリティ](https://code.claude.com/docs/ja/security) / [プライバシーポリシー](https://docs.anthropic.com/ja/docs/legal-center/privacy)）。法人利用では、便利さより先に情報の境界線を決めます。
...
法人研修では、まず `./CLAUDE.md` に部署・プロジェクト共通ルールを書きます。`CLAUDE.local.md` や `~/.claude/CLAUDE.md` は、共通ルールを壊さない範囲の個人補足として扱います。
...
├── .mcp.json              # MCP 定義（ルート直下。.claude/ の中ではない）
└── .claude/
    ├── settings.json      # 権限・フックなど動作設定（チーム共有）
    ├── settings.local.json
    ├── skills/            # 定型手順（1 スキル 1 フォルダ、SKILL.md）
    ├── commands/          # 旧形式（後方互換）。新規は skills/ を使う
    ├── agents/            # サブエージェント定義
...
> [!NOTE]
> 社内で MCP を使わない場合は `.mcp.json` は不要です。`output-styles/` や `plugins/` などもありますが、本研修の初回導入では必須ではありません。
...
| 強い ← → 弱い | 内容 |
|---|---|
| Managed settings | 組織が配布する設定（個人では上書きできない） |
...
| 部品 | 役割 | 使いどころ |
|---|---|---|
| Skills | 手順・知識のパッケージ（`.claude/skills/各フォルダ/SKILL.md`）。**新規はこちらを推奨** | 研修資料レビュー、議事録整理など |
| Commands | 短い定型プロンプトの呼び出し（`.claude/commands/*.md`） | `/review` のような 1 行コマンド |
...
`/context` を実行すると、コンテキストの使用量が表示されます。上記の対処の流れの 1 番目として使い、圧縮やリセットが必要か判断してください。
...
`/clear` とは異なり、会話の流れを保ったまま容量だけ圧縮できます。コンテキストが上限に近づくと、自動でコンパクションが走る場合もあります。

#### 会話をリセットする（`/clear`）

別のタスクに切り替えるときは `/clear` を使います。会話履歴だけが空になり、新しい作業を始めます。`CLAUDE.md` やプロジェクトの設定ファイルは残ったままです。`/compact` とは違い、これまでの会話の流れは引き継がれません。

#### セッション再開（`/resume`）

`/resume` はコンテキストの圧縮・リセットとは目的が異なります。前回のセッションを会話履歴ごと引き継いで再開するコマンドです。ターミナルを閉じた後や別の作業に割り込んだ後でも、前の文脈をそのまま復元できます。
...
| `/clear` | 会話履歴のリセット（別のタスクに切り替えるとき。§9 参照） |
| `/context` | コンテキスト使用量の確認（会話ログ・読み込みファイルの割合など） |
...
**A.** 研修中は入力しないでください。Claude Code はローカル PC 上で動作しますが、プロンプトとレスポンスは Anthropic の API を経由します（[Claude Code セキュリティ](https://code.claude.com/docs/ja/security)）。社内のセキュリティポリシーに従い、研修では必ずダミーデータを使います。
