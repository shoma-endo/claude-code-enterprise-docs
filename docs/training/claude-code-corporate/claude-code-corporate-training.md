...
| 回 | タイトル | 時間 | 到達目標 |
|---|---|---:|---|
| Session 1 | Claude Code の正体と基本操作 | 5 時間 | 安全ルールを理解し、ファイル作成・読取・編集を完走する |
...
2. 表示された指示に従い、**サインイン（ログイン）** する

分からないときは、まず社内の IT 窓口または研修講師に相談してください。次の表は用語のイメージです。
...
一方で **`npm install -g @anthropic-ai/claude-code` で入れる方法** は公式にも残っており、その場合は **Node.js 18 以上** が条件です（[高度なセットアップ](https://code.claude.com/docs/ja/setup)）。社内で「npm 経由のみ可」と決まっているときは IT の指示に従ってください。いずれの入れ方でも、公式の説明では **動いている `claude` 本体は実行時に Node を呼び出さない** ネイティブバイナリです。

### 受講者の準備（研修の 1 週間前まで）

#### 0. 社内ルールの確認（必須）

社内規程に従い、**外部の AI に入力してよい情報・してはいけない情報** を確認してください。不明な場合は上司または IT に聞いてから進めます。
...
実行中に「続行しますか」などと聞かれたら、画面の案内に従って許可します。 **会社の PC で弾かれる場合は、自分で無理に許可せず、IT に依頼** してください。
...
数字の並びが違っても問題ありません。**`Claude Code` の文字列や、ピリオドで区切られたバージョンらしき数字**が含まれていれば、コマンドが認識できていることが多いです。
...
うまくいかない場合は、[インストールのトラブルシューティング（日本語）](https://code.claude.com/docs/ja/troubleshoot-install) を開き、手順どおり試すか、IT・講師に相談してください。
...
> **時間**: 5 時間
> **ゴール**: Claude Code の正体、法人利用の安全境界、起動方法、ファイル操作、エージェントループを理解する
...
| 2:30–3:05 | 許可ダイアログ、停止方法、復旧方法 | 実習 |
| 3:05–3:15 | 休憩 | - |
| 3:15–4:10 | ファイル作成・読取・編集 | 実習 |
| 4:10–4:40 | エージェントループとチェックポイント | 講義・実習 |
...
|---|---|
| `Esc` | 実行中の作業を止めたい |
| `Esc` 2 回 | チェックポイントから巻き戻したい |
| `/exit` | セッションを終了したい |
| `/clear` | 会話履歴を捨てて新しく始めたい |
...
```mermaid
flowchart TD
  A["① コンテキスト収集<br/>ファイルを読む・検索する"] --> B["② アクション実行<br/>編集する・コマンドを実行する"]
  B --> C["③ 結果の検証<br/>出力を見る・必要なら修正する"]
  C -->|"作業継続"| A
  C -->|"完了"| D["終了"]
```
...
Claude Code はファイル編集前にチェックポイントを作ることがあります。これは、セッション内で「直前の状態に戻す」ための仕組みです。
...
| 症状 | 原因候補 | 対応 |
|---|---|---|
| `claude: command not found` | 未インストール、ターミナルを開き直していない、PATH が通っていない | ターミナルを一度閉じて開き直す。それでも出る場合は [インストールのトラブルシューティング（日本語）](https://code.claude.com/docs/ja/troubleshoot-install) に従うか、IT・講師に相談 |
...
| 1:45–1:55 | 休憩 | - |
| 1:55–2:35 | Skills / Commands / Agents / Hooks | 講義 |
| 2:35–3:05 | コンテキスト管理と Plan モード | 講義・実習 |
| 3:05–3:15 | 休憩 | - |
...
### 7. Skills / Commands / Agents / Hooks
...
- タスクが変わったら `/clear` する
- **コンパクション**とは、会話履歴を要約してコンテキストを空け直す操作です。長い作業の途中では `/compact` で行います
...
| 3:40–3:50 | 休憩 | - |
| 3:50–4:30 | 定着計画作成 | 実習 |
| 4:30–5:00 | 成果の整理、クロージング | 共有 |

### ワーク 1: 議事録整理

以下のサンプル議事録を使います。実案件情報は使わないでください。すでに Session 1 で同じフォルダに `meeting-summary.md` がある場合は、保存ファイル名を `meeting-summary-work3.md` などに変えても構いません（プロンプト内の保存先名を合わせてください）。
...
```text
「業務効率化のための生成AI導入」について、社内提案書の骨子を作ってください。
...
研修後に使わなければ、学習は業務成果に変わりません。最後に 1 週間の定着計画を作ります。
...
| ファイル | 場所 | 役割 |
|---|---|---|
| `CLAUDE.md` | プロジェクトルート or `~/.claude/` | Claude への指示・ルール |
| `settings.json` | `.claude/` or `~/.claude/` | 許可設定・環境変数・フック登録 |
| `commands/` | `.claude/` or `~/.claude/` | 独自スラッシュコマンド |
| `skills/` | `.claude/` or `~/.claude/` | スキル定義 |
| `agents/` | `.claude/` or `~/.claude/` | サブエージェント定義 |
| `hooks/` | `.claude/` or `~/.claude/` | 自動実行処理 |
